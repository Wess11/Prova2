...
class gerirTimes
{
	...
	function cadTime($nome, $participantes, $esporte, $genero) {
		$conecta = mysqli_connect(bd_serv, bd_usuario, bd_senha, bd_banco);
		$campos = "nome, participantes, esporte, genero";
		$valores = "'$nome', '$participantes', '$esporte', '$genero'";
	...
	function updTime($id, $nome, $participantes, $esporte, $genero) {
		$conecta = mysqli_connect(bd_serv, bd_usuario, bd_senha, bd_banco);
		$sql = "UPDATE tbdtimes SET nome = '$nome', participantes = '$participantes', esporte = '$esporte', genero = '$genero' WHERE id = '$id'";
		$query = mysqli_query($conecta, $sql);
		return $query;
		//mysqli_close($conecta);
	}
